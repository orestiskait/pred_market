...
case "$cmd" in
  stop)
    echo "[start_stop_weather_bot] Stopping weather bot gracefully..."
    $DOCKER stop --time 30 "$CONTAINER" 2>/dev/null || echo "(weather-bot not running)"
    ;;
  ...
  start)
    ...
    echo "[start_stop_weather_bot] Checking if container is running..."
