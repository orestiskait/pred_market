...
1. **Kalshi API:** Get API key ID + private key PEM from Kalshi. Copy PEM to `~/.kalshi/kalshi_api_key.txt` on the VM. Run `first_time_vm_setup.sh` (prompts for key ID, creates `kalshi_api_key_id`).
