...
class NWPPointFetcher:
    ...
    def _extract_from_dataset(
        ...
        for step_val in steps:
            ...
            for stn in stations:
                ...
                cycle_ts = (
                    pd.Timestamp(cycle).tz_localize("UTC")
                    if cycle.tzinfo is None
                    else pd.Timestamp(cycle)
                )
                valid_ts = (
                    pd.Timestamp(valid_utc).tz_localize("UTC")
                    if valid_utc.tzinfo is None
                    else pd.Timestamp(valid_utc)
                )
                rows.append({
                    "station": stn.icao,
                    "city": stn.city,
    ...
    def fetch_date_range(
        ...
        rolling_lead_minutes : int, optional
            If set, use rolling strategy: fetch only the fxx containing this lead,
            and filter to this forecast_minutes. E.g. 120 = 2h-out forecast.
        """
        if cycles is None:
            cycles = list(self.DEFAULT_CYCLES)

        if rolling_lead_minutes is not None:
            # fxx that contains this lead: 120 min → f02 (steps 75–120)
