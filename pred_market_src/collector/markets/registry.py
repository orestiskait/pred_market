...
Expanding to new cities / markets:
  ...
  2. That's it — every service (collector, listener, bot, weather fetchers)
     picks up the new market automatically via config.yaml `event_series`.

Design notes
------------
* ``StationInfo`` is kept for backward compatibility with the ``weather/``
  fetcher package, which only needs ICAO / IATA / city / tz.
* ``MarketConfig`` adds the Synoptic push station ID and the event-series
  prefix so that a single object fully describes a market.
...
class StationInfo:
    ...
    This is intentionally a *subset* of MarketConfig so that the weather/
    fetcher package (which doesn't care about Kalshi or Synoptic push IDs)
    can use a lightweight object.
    """

    icao: str       # 4-letter ICAO id  (e.g. "KMDW")
    iata: str       # 3-letter FAA/IATA (e.g. "MDW") — used by IEM ASOS API
    city: str       # Human-readable city name
...
class MarketConfig:
    ...
    def station_info(self) -> StationInfo:
        """Return a lightweight StationInfo (for weather fetchers)."""
        return StationInfo(
            icao=self.icao,
            iata=self.iata,
            city=self.city,
...
# Build a StationInfo-compatible registry for backward compat with weather/
STATION_REGISTRY: dict[str, StationInfo] = {
    k: v.station_info for k, v in MARKET_REGISTRY.items()
}
